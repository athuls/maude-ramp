...
do
	...
	../maude-binaries/alpha118/maude init-client"$clientid"_"$iter".maude > /proj/Confluence/maude/debug_logs/temp/client_logs/maude_client"$clientid"_logs_"$iter".txt 2>&1 &
	sleep 1
	iter=`expr $iter + 1`
done
