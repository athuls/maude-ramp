...
class ClientThread extends Thread {
    ...
    public ClientThread(DB db, boolean dotransactions, Workload workload, int threadid, int threadcount,
                        ...
                        Semaphore startSem, Semaphore initializedSem) {
...
//                int numInserts;
        ...
        try {
            //System.out.println(_threadid + " is stopping!");
            _db.cleanup();
        } catch (DBException e) {
            e.printStackTrace();
            e.printStackTrace(System.out);
...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        Semaphore threadStartSem = new Semaphore(0);
        Semaphore threadInitializedSem = new Semaphore(0);
        ...
        for (int threadid = 0; threadid < threadcount; threadid++) {
            ...
            Thread t =
                    new ClientThread(db, dotransactions, workload, threadid, threadcount, props, opcount / threadcount,
                                     targetperthreadperms,
                            threadStartSem, threadInitializedSem);
        ...
        threadInitializedSem.acquireUninterruptibly(threadcount);

        long st = System.currentTimeMillis();

        threadStartSem.release(threadcount);
